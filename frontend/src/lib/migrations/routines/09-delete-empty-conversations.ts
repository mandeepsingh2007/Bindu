...
type Collection<T> = { find: (filter: Record<string, unknown>, options?: Record<string, unknown>) => FindCursor<T>; deleteMany: (filter: Record<string, unknown>) => Promise<{ deletedCount: number }> };
type FindCursor<T> = { batchSize: (n: number) => FindCursor<T>; hasNext: () => Promise<boolean>; next: () => Promise<T | null>; toArray: () => Promise<T[]> };
